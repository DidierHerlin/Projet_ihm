    ...
    <div class="body-wrapper">
      ...
      <div class="container-fluid">
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title fw-semibold mb-4">Modifier voyage</h5>
              <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('modifierVoyage', ['id' => $voyages->id]) }}">
                        @csrf
                        <div class="mb-3">

                            <input type="hidden" class="form-control" id="exampleInputEmail1" value="{{$voyages->id}}" aria-describedby="emailHelp" name="id">
                        </div>
                        <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label">Date</label>
                        <input type="date" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="date_voyage" value="{{$voyages->date_voyage}}">
                        </div>
                        <div class="mb-3">
                            <label for="conducteur_id" class="form-label">Conducteur</label>
                            <select class="form-select" id="conducteur_id" name="id_conducteur"
                                aria-describedby="conducteurHelp">
                                @isset($conducteur)
                                    @foreach ($conducteur as $conducteur)
                                        <option value="{{ $conducteur->id }}">
                                            {{ $conducteur->nom_conducteur }}</option>
                                    @endforeach
                                @endisset
                            </select>
                            <div class="mb-3">
                            <label for="conducteur_id" class="form-label">Véhicule</label>
                            <select class="form-select" id="conducteur_id" name="id_vehicule"
                                aria-describedby="conducteurHelp">
                                @isset($vehicules)
                                    @foreach ($vehicules as $vehicules)
                                        <option value="{{ $vehicules->id }}">
                                            {{ $vehicules->immatricule }}</option>
                                    @endforeach
                                @endisset
                            </select>
                            <div class="mb-3">
                                <label for="conducteur_id" class="form-label">Trajet</label>
                                <select class="form-select" id="conducteur_id" name="id_trajet"
                                    aria-describedby="conducteurHelp">
                                    @isset($trajets)
                                        @foreach ($trajets as $trajets)
                                            <option value="{{ $trajets->id }}">
                                                {{ $trajets->ville_depart . ' - ' . $trajets->ville_arriver }}</option>
                                        @endforeach
                                    @endisset
                                </select>
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">Nombre de passage</label>
                            <input type="text" class="form-control" id="exampleInputPassword1" name="nombre_voageur" value="{{$voyages->nombre_voageur}}">
                        </div>
