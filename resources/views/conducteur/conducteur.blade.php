            ...
            <div class="col-12">
                <div class="card w-100">
                    <div class="card-body p-4">
                      <a href="/conducteur/ajout_conducteur"><button type="button" class="btn btn-primary m-1">Ajouter</button></a>

                        <h5 class="card-title fw-semibold mb-4">Liste de conducteur</h5>
                        <div class="table-responsive">
                                ...
                                <!-- Le contenu de votre tableau ici -->
                                    ...
                                    <table class="table text-nowrap mb-0 align-middle">
                                      ...
                                      <tbody>
                                        @foreach($conducteurs as $conducteurs)
                                        <tr>

                                            <td>{{$conducteurs->cin}}</td>
                                            <td>{{$conducteurs->nom_conducteur}}</td>
                                            <td>{{$conducteurs->telephone}}</td>
                                            <td>{{$conducteurs->adresse}}</td>

                                            <td>
                                                <button class="btn btn-success">
                                                    <a href="/conducteur/modifier_conducteur/{{$conducteurs->id}}" target="_blank" rel="noopener noreferrer">
                                                    ...
                                                    <form id="deleteForm"  action="{{route('supprimerConducteur',$conducteurs->id)}}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
  ...
  {{-- <script>
      @if(session('success'))
          Swal.fire({
              ...
              icon: 'success',
              timer: 3000 // temps en millisecondes (3 secondes)
          });
      @endif
  </script> --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        document.getElementById('deleteForm').addEventListener('submit', function (event) {
            ...
            Swal.fire({
                title: 'Voulez-vous vraiment supprimer les données?',
                ...
                if (result.isConfirmed) {
                    ...
                    // Afficher le message de succès après la soumission du formulaire
                    Swal.fire({
                    title: 'Suppression réussie!',
                    text: 'Les données ont été supprimées avec succès.',
                    icon: 'success',
                    showConfirmButton: 'OK' // Cette option cache le bouton de confirmation
                });
