    ...
    <div class="body-wrapper">
      ...
      <div class="container-fluid">
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title fw-semibold mb-4">Modifier trajet</h5>
              <div class="card">
                <div class="card-body">
                    <div class="card-body">
                        <form action="{{ route('modifTrajet', ['id' => $trajets->id]) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <!-- Supprimez le champ caché si vous utilisez le modèle $trajets -->
                                <input type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="id" value="{{$trajets->id}}">
                            </div>
                            <div class="mb-3">
                                <label for="ville_depart" class="form-label">Départ</label>
                                <input type="text" class="form-control" id="ville_depart" aria-describedby="emailHelp" value="{{ $trajets->ville_depart }}" name="ville_depart">
                            </div>
                            <div class="mb-3">
                                <label for="ville_arriver" class="form-label">Arriver</label>
                                <input type="text" class="form-control" id="ville_arriver" value="{{ $trajets->ville_arriver }}" name="ville_arriver">
                            </div>
                            <div class="mb-3">
                                <label for="distance" class="form-label">Distance</label>
                                <input type="text" class="form-control" id="distance" value="{{ $trajets->distance }}" name="distance">
                            </div>
                            <div class="mb-3">
                                <label for="montant_trajet" class="form-label">Montant</label>
                                <input type="text" class="form-control" id="montant_trajet" value="{{ $trajets->montant_trajet }}" name="montant_trajet">
                            </div>
