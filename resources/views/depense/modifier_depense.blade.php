    ...
    <div class="body-wrapper">
      ...
      <div class="container-fluid">
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title fw-semibold mb-4">Modifier recettes</h5>
              <div class="card">
                <div class="card-body">
                    <form action="{{route('modifierDepense',['id' => $depenses->id] )}}" method="POST">
                        @csrf
                        <div class="mb-3">

                            <input type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="id" value="{{$depenses->id}}">
                          </div>
                        <div class="mb-3">
                          <label for="exampleInputEmail1" class="form-label">Date</label>
                          <input type="date" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="date_depense" value="{{$depenses->date_depense}}">
                        </div>
                        <div class="mb-3">
                            <label for="conducteur_id" class="form-label">Conducteur</label>
                            <select class="form-select" id="id_conducteur" name="id_conducteur"
                                aria-describedby="conducteurHelp">
                                @isset($conducteur)
                                    @foreach ($conducteur as $conducteur)
                                    <option value="{{ $conducteur->id}}"{{ $conducteur->id == $conducteur->id ? 'selected' : '' }}>{{ $conducteur->nom_conducteur }}</option>
                                    @endforeach
                                @endisset
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="conducteur_id" class="form-label">Véhicule</label>
                            <select class="form-select" id="id_vehicule" name="id_vehicule"
                                aria-describedby="conducteurHelp">
                                @isset($vehicules)
                                    @foreach ($vehicules as $vehicules)
                                    <option value="{{ $vehicules->id}}"{{ $vehicules->id == $vehicules->id ? 'selected' : '' }}>{{ $vehicules->immatricule }}</option>
                                    @endforeach
                                @endisset
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="exampleSelect" class="form-label">Type</label>
                            <input type="texte" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="type_depense" value="{{$depenses->type_depense}}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="exampleInputPassword1" name="montant_depense" value="{{$depenses->montant_depense}}">
                          </div>
